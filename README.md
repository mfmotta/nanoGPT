...
Registers -> Local Cache (Shared Memory and L1/Read-only Cache) -> L2 Cache -> GPU DRAM (Global Memory + Constant Memory + Read-only/Texture Memory)
...
- high bandwidth memory (HBM): 40-80GB with bandwidth 1555 GB/sec (this is the GPU DRAM --aka 'global' memory*footnote: (‘local’ and ‘constant’ memory are just different addressing modes for global memory, also DRAM))
...
"The number of blocks and warps that can reside and be processed together on the multiprocessor for a given kernel depends on the amount of registers and shared memory used by the kernel and the amount of registers and shared memory available on the multiprocessor." (https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#hardware-multithreading)
