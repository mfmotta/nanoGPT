...
#exec(open('configurator.py').read()) # overrides from command line or config file #MM:not using this
...
ddp = True # use ddp even though we have a single GPU -- for practice
if ddp:
    ...
    ddp_local_rank = 0 
    ddp_world_size = 1 
    init_process_group(backend='', rank=ddp_rank, world_size=ddp_world_size)
